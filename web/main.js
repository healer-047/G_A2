// NYC Yellow Taxi 2023 — Interactive Trip Explorer (D3 v7 + coordinated views)
//
// This dashboard is designed for exploratory analysis:
// - When is demand highest (by date / weekday / hour)?
// - How do distance and total amount relate? Where are outliers?
// - How do payment types differ (distributions)?
// - What are the dominant pickup→dropoff corridors (Sankey)?
//
// Data inputs are pre-aggregated for performance:
//   data/daily.csv       : daily aggregates
//   data/daily_hour.csv  : (date, hour) aggregates
//   data/sample.csv      : sampled trips for point-based views (scatter, box, sankey details)
//   data/meta.json       : dataset metadata
//
// Interactions (meets assignment requirements):
// - Brushing on time series to set date range (dynamic query)
// - Brushing on scatterplot (2D) to set distance×total range (brushing & linking)
// - Brushing on histogram to set distance range (bidirectional with scatterplot)
// - Clicking heatmap cell to filter weekday+hour
// - Clicking box plot to filter payment type (also selectable from dropdown)
// - Clicking Sankey nodes to filter pickup or dropoff zone
// - Tooltips on hover (points, bars, heatmap cells, Sankey links/nodes)
...
(function () {
  ...
  function TimeSeriesChart(svgSel) {
    ...
    const pathArea = g.append("path").attr("fill", "rgba(138,180,248,0.20)");
    const pathLine = g.append("path").attr("fill", "none").attr("stroke", "rgba(138,180,248,0.95)").attr("stroke-width", 2);

    const focusG = g.append("g").style("display", "none");
    const focusLine = focusG.append("line").attr("y1", 0).attr("y2", 1).attr("stroke", "rgba(230,232,239,0.65)").attr("stroke-dasharray", "3,3");
    const focusDot = focusG.append("circle").attr("r", 4).attr("fill", "rgba(230,232,239,0.85)");
    ...
    function update() {
      ...
      gGrid.selectAll("line")
        ...
        .attr("y1", (t) => y(t))
        .attr("y2", (t) => y(t))
        .attr("stroke", "rgba(230,232,239,0.09)");
    ...
    function update() {
      ...
      sel.join(
        (enter) => enter.append("rect")
          ...
          .attr("stroke", "rgba(230,232,239,0.14)")
          ...
          .on("mouseenter", (event, d) => {
            showTooltip(
              `<div class="tt-title">${DOW[d.dow]} @ ${d.hour}:00</div>
               ...
               <div>Avg distance: <strong>${fmt1(d.avg_distance)}</strong> mi</div>
               <div>Avg total: <strong>$${fmt2(d.avg_total)}</strong></div>
               <div style="margin-top:6px;color:rgba(230,232,239,0.75)">Click to filter hour+weekday</div>`,
              event.clientX,
              event.clientY
            );
          })
    ...
    function update() {
      ...
      xLab.join(
        (enter) => enter.append("text")
          ...
          .attr("fill", "rgba(230,232,239,0.80)")
          .attr("font-size", 12)
      ...
      sel.join(
        (enter) => enter.append("circle")
          ...
          .attr("r", 2.2)
          .attr("fill", (d) => (inSel(d) ? color(d.payment_type) : "rgba(230,232,239,0.25)"))
      ...
      liEnter.append("text").attr("x", -18).attr("y", 4).attr("text-anchor", "end")
        .attr("fill", "rgba(230,232,239,0.75)").attr("font-size", 11);
    ...
    function update() {
      ...
      bars.join(
        (enter) => enter.append("rect")
          ...
          .attr("fill", "rgba(138,180,248,0.30)")
          .attr("stroke", "rgba(138,180,248,0.70)")
          ...
          .on("mouseenter", (event, d) => {
            showTooltip(
              `<div class="tt-title">Distance bin</div>
               ...
               <div style="margin-top:6px;color:rgba(230,232,239,0.75)">Brush to filter distance</div>`,
              event.clientX,
              event.clientY
            );
          })
  ...
  function BoxPlotChart(svgSel) {
    ...
    function update() {
        ...
        .on("mouseenter", (event, d) => {
          showTooltip(
            `<div class="tt-title">${paymentLabel(d.pt)}</div>
             ...
             <div style="margin-top:6px;color:rgba(230,232,239,0.75)">Click to filter payment type</div>`,
            event.clientX,
            event.clientY
          );
        })
      ...
      merged.select("line.whisker")
        ...
        .attr("stroke", "rgba(230,232,239,0.55)");
      ...
      merged.select("rect.boxrect")
        ...
        .attr("fill", (d) => {
          ...
          return selected ? "rgba(255,255,255,0.24)" : "rgba(138,180,248,0.22)";
        })
        .attr("stroke", (d) => {
          const selected = (state.paymentType !== "all" && +state.paymentType === d.pt);
          return selected ? "rgba(255,255,255,0.90)" : "rgba(138,180,248,0.70)";
        });

      merged.select("line.median")
        .attr("x1", 0).attr("x2", bw)
        .attr("y1", (d) => y(d.median)).attr("y2", (d) => y(d.median))
        .attr("stroke", "rgba(230,232,239,0.85)")
        .attr("stroke-width", 2);
    ...
    function update() {
      ...
      linkSel.join(
        (enter) => enter.append("path")
          ...
          .attr("fill", "none")
          .attr("stroke", "rgba(138,180,248,0.35)")
          ...
          .on("mouseenter", (event, d) => {
            showTooltip(
              `<div class="tt-title">Corridor</div>
               ...
               <div style="margin-top:6px;color:rgba(230,232,239,0.75)">Click a node to filter PU/DO</div>`,
              event.clientX,
              event.clientY
            );
          })
      ...
      nodeEnter.append("text").attr("dy", "0.35em").attr("font-size", 11).attr("fill", "rgba(230,232,239,0.85)");
      ...
      nodeMerged.select("rect")
        ...
        .attr("fill", (d) => d.side === "pu" ? "rgba(138,180,248,0.38)" : "rgba(230,232,239,0.18)")
        .attr("stroke", (d) => {
          const selected = (d.side === "pu" && state.pu === d.id) || (d.side === "do" && state.do === d.id);
          return selected ? "rgba(255,255,255,0.95)" : "rgba(230,232,239,0.24)";
        })
        .attr("stroke-width", (d) => {
          const selected = (d.side === "pu" && state.pu === d.id) || (d.side === "do" && state.do === d.id);
          return selected ? 2 : 1;
        ...
        .on("mouseenter", (event, d) => {
          ...
          showTooltip(
            `<div class="tt-title">${d.name}</div>
             ...
             <div style="margin-top:6px;color:rgba(230,232,239,0.75)">
