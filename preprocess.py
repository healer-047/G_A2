#!/usr/bin/env python3
"""
Preprocess NYC Yellow Taxi trip data for the D3 dashboard.

Why?
- The raw CSV is large (~1M rows). Aggregating + sampling makes the web UI fast.
- The dashboard uses:
  * daily.csv       (date-level time series)
  * daily_hour.csv  (date×hour aggregates used for an interactive hour×weekday heatmap)
  * sample.csv      (sampled trips for scatter, box plot, sankey)
  * meta.json       (basic metadata)

Usage
-----
python preprocess.py --input 2023_Yellow_Taxi_Trip_Data.csv --out web/data --sample 50000

Notes
-----
- This script performs light cleaning:
  * Valid pickup/dropoff datetimes
  * 0 < duration <= 180 minutes
  * 0 < trip_distance <= 50 miles
  * 0 < total_amount <= 500 USD
- Adjust thresholds if your assignment expects a different cleaning policy.
"""
import argparse
import json
